        ...
        <div>
            {userDat.status == "disabled" ? <Login /> :
                <div><h3>Welcome {userDat.name}</h3>
                    ...
                    {pass ?<form onSubmit={(e)=>handleEdit(e)}> <label>Password:</label><input type="text" placeholder="password" onChange={(e)=>{setUserDat({...userDat, password: e.target.value})}} /> <br />
                    
                        <button className="btn btn-success" type="submit"></button>
                        </form> : null}

                    <button className="btn btn-warning m-1" onClick={() => setEdit(true)}>Edit</button>
